...
import {
  mockGetLocalAlerts, mockGetNationalTrends, mockSubmitReport, mockGetBroadcasts,
  LGA_COORDS,
        ...
        {symptoms.length > 0 && (
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {symptoms.map(s => <span key={s} className="text-xs bg-white border border-slate-200 text-slate-700 px-2.5 py-1 rounded-full font-medium shadow-sm">{s}</span>)}
            </div>
          </div>
        )}
        ...
        <div className="px-5 pb-5 border-t border-white/60 pt-4 space-y-2">
          {alert.status === 'confirmed' ? (
            <ul className="space-y-1.5 text-sm text-slate-700">
              ...
              <li>📞 If symptoms are severe, call your local health emergency line</li>
            </ul>
            ...
            <ul className="space-y-1.5 text-sm text-slate-700">
              ...
              <li>🏥 See a doctor early if you develop symptoms</li>
            </ul>
          ) : (
            <ul className="space-y-1.5 text-sm text-slate-700">
...
function ReportModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: (lga: string, report: any) => void }) {
  ...
  const submit = async () => {
    ...
    const match = Object.entries(LGA_COORDS).find(([k]) => lgaKey.includes(k));
    const coords = match ? match[1] : { lat: 6.524 + (Math.random() - 0.5) * 0.1, lng: 3.379 + (Math.random() - 0.5) * 0.1 };
    const report = mockSubmitReport({
      source: 'community',
      patient_count: 1,
      symptom_matrix: selectedSymptoms.map(s => s.toLowerCase()),
      severity,
      origin_address: `${lga.trim()}, Lagos`,
      origin_lat: coords.lat,
      origin_lng: coords.lng,
      lga: lga.trim(),
    });
    onSuccess(lga.trim(), report);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative z-10 bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Symptoms <span className="text-red-500">*</span></label>
          ...
          <div>
            ...
            <input type="range" min={1} max={10} value={severity} onChange={e => setSeverity(Number(e.target.value))} className="w-full accent-blue-600" />
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-3xl mx-auto px-5 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-base tracking-tight text-slate-900">DOMRS</span>
            ...
            <div className="flex items-center gap-1 ml-1">
              ...
              <span className="text-xs text-slate-400 hidden sm:block">Offline demo</span>
            </div>
          </div>
          <SyncStatusBadge />
      ...
      <main className="max-w-3xl mx-auto px-5 py-8 space-y-8">
            ...
            {broadcasts.map(bc => (
              <div key={bc.id} className={`rounded-2xl border p-5 flex items-start gap-4 ${
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-white/60 mt-1">From {bc.issued_by} · {bc.zone}</p>
                </div>
              </div>
            ))}
          </section>
        ...
        <div className="bg-blue-600 rounded-2xl p-5 text-white flex items-center justify-between gap-4 shadow-lg shadow-blue-600/25">
          <div>
            ...
            <p className="text-blue-100 text-sm mt-0.5">Report illness in your community — completely anonymous.</p>
          </div>
        ...
        <section style={{ isolation: 'isolate' }}>
          <button
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold px-2.5 py-1 rounded-full border bg-green-50 border-green-200 text-green-700">● Live</span>
            </div>
            <span className={`text-xs font-semibold text-slate-400 group-hover:text-slate-700 flex items-center gap-1 transition-colors`}>
              {mapOpen ? 'Hide map' : 'Show map'}
              <svg className={`w-3.5 h-3.5 transition-transform ${mapOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
            </span>
          </button>
          {mapOpen && (
            <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-sm">
              <CivilianMap ref={mapRef} height={300} showLegend />
            </div>
          )}
        </section>
        ...
        {/* High-risk banner */}
        {hasHighRisk && (
          <div className="rounded-2xl bg-red-600 text-white p-5 flex items-start gap-4">
        ...
        {alerts.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-semibold text-slate-500">{alerts.length} active</span>
            </div>
        ...
        <footer className="text-center text-xs text-slate-300 pt-4 pb-8 space-y-1">
          <p className="font-semibold">DOMRS — Disease Outbreak Monitoring &amp; Response System</p>
          <p>Demo mode — all data is local and offline.</p>
        </footer>
      </main>
      ...
      {showReport && (
        <ReportModal
          ...
          onSuccess={(lga, report) => {
            setShowReport(false);
            setAlerts(mockGetLocalAlerts());
            setTrends(mockGetNationalTrends());
            // Push live pin onto map
            mapRef.current?.addPin(report);
            setSuccessMsg(`Your report for ${lga} has been submitted. Thank you for helping keep your community safe!`);
